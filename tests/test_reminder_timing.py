...
class ReminderTimingTests(unittest.TestCase):
    ...
    def test_early_schedule_window_waits_until_target_time(self):
        now = self.module.parse_shanghai_time("2026-06-22 20:24:00")
        ...
        self.assertEqual(self.module.seconds_until_target(reminder, now), 36 * 60)

    def test_late_schedule_window_sends_immediately(self):
        now = self.module.parse_shanghai_time("2026-06-22 12:19:30")

        reminder = self.module.select_scheduled_reminder(now)

        self.assertEqual(reminder["key"], "lunch")
    ...
    def test_after_late_window_is_skipped(self):
        now = self.module.parse_shanghai_time("2026-06-22 22:00:00")
        self.assertEqual(self.module.select_scheduled_reminder(now)["key"], "recovery")

        too_late = self.module.parse_shanghai_time("2026-06-22 22:00:01")
